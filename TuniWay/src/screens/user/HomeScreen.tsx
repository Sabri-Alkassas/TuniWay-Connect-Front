...
function QuickCard({ emoji, label, sub, bg, onPress }: QuickCardProps) {
  return (
...
interface RecentTicketRowProps { ticket: ClientTicketDto; }
function RecentTicketRow({ ticket }: RecentTicketRowProps) {
...
export function HomeScreen() {
  ...
  const displayName = [data?.firstName, data?.lastName].filter(Boolean).join(' ') || 'You';
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.hero}>
        ...
        {loading && !data ? (
          ...
          <View style={s.heroError}>
            <Text style={s.heroErrorTitle}>We could not load your dashboard</Text>
            <Text style={s.heroErrorText}>{error ?? 'Please try again in a moment.'}</Text>
            <TouchableOpacity style={s.heroRetryBtn} onPress={() => load()} activeOpacity={0.8}>
              <Text style={s.heroRetryTxt}>Retry</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <View style={s.heroStats}>
            <StatTile label="Trips" value={data.totalTrips ?? 0} />
            <StatTile label="Active tickets" value={data.activeTickets ?? 0} />
            <StatTile
              label="Account"
              value={data.accountStatus === 'ACTIVE' ? '✓ Active' : 'Check'}
            />
          </View>
        )}
      </View>
      ...
      >
        ...
        {(data?.missingFields?.length ?? 0) > 0 && (
          ...
          >
            ...
            <Text style={s.warnTxt}>
              Complete your profile: {data?.missingFields.join(', ')}
            </Text>
            <Text style={s.warnArrow}>›</Text>
          </TouchableOpacity>
        )}
        ...
        <View style={s.qaGrid}>
          <QuickCard
            emoji="🔍" label="Find a Route" sub="Search lines"
            bg={colors.blueLt}
            onPress={() => navigation.navigate('UserTabs', { screen: 'Search' })}
          />
          <QuickCard
            emoji="📍" label="Nearby Stops" sub="Map view"
            bg={colors.redLt}
            onPress={() => navigation.navigate('UserTabs', { screen: 'Map' })}
          />
          <QuickCard
            emoji="🎟" label="My Tickets" sub="History"
            bg={colors.greenLt}
            onPress={() => navigation.navigate('UserTabs', { screen: 'Tickets' })}
          />
          <QuickCard
            emoji="👤" label="My Profile" sub="Edit info"
            bg="#f5e6ff"
            onPress={() => navigation.navigate('EditProfile')}
          />
        </View>
...
const s = StyleSheet.create({
  ...
  warnArrow:     { fontSize: 18, color: '#7a5700', fontWeight: '700' },
